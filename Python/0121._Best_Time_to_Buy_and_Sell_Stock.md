...
对于除了最后一天的每一天i，假设我们当天买入，如果我们能知道后面所以日子里面哪一天j price最高，那我们就第i天买入，在第j天卖出，这样算出一个最大值
...
class Solution:
    def maxProfit(self, prices: List[int]) -> int:
        ...
        maxx = [] # 算出每一天后面的price的最大值
        for i in range(len(prices)):
            if not maxx:
                maxx.append(prices[-i-1])
            else:
                maxx.append(max(maxx[-1], prices[-i-1]))
        maxx = maxx[::-1]
        
        return max(max([maxx[i+1] - prices[i] for i in range(len(prices)-1)]), 0)
```
